...
class FeatureTransformer(BaseFeatureTransformer):
    def forward(self, feature_indices):
        return SparseLinearFunction.apply(feature_indices, self.weight, self.bias)


class DoubleFeatureTransformer(BaseFeatureTransformer):
    def forward(self, feature_indices_0, feature_indices_1):
        return (
            SparseLinearFunction.apply(feature_indices_0, self.weight, self.bias),
            SparseLinearFunction.apply(feature_indices_1, self.weight, self.bias),
        )
